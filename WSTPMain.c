...
void initFineFromCoarse P(( int _tp1, int _tp2));
...
void computeArtificialLighting P(( int _tp1, double * _tp2, long _tpl2));
...
#endif
{
	...
	computeArtificialLighting(_tp1, _tp2, _tpl2);
...
void renderScene P(( int _tp1, const char * _tp2));
...
#endif
{
	...
	renderScene(_tp1, _tp2);

	res = 1;
	WSReleaseString(mlp, _tp2);
L1: 
L0:	return res;
} /* _tr22 */
...
static struct func {
	...
	const char  *f_name;
		...
		{ 0, 0, _tr0, "Get42" },
		{ 1, 0, _tr1, "createScene" },
		{ 1, 0, _tr2, "getSceneVoxelSize" },
		{ 1, 0, _tr3, "sceneExistsQ" },
		{ 1, 0, _tr4, "initAD" },
		{ 2, 0, _tr5, "initFineADFromCoarseAD" },
		{ 2, 0, _tr6, "serializeScene" },
		{ 2, 0, _tr7, "deserializeScene" },
		{ 1, 0, _tr8, "countVoxelBlocks" },
		{ 2, 0, _tr9, "getVoxelBlock" },
		{ 1, 2, _tr10, "putVoxelBlock" },
		{ 2, 0, _tr11, "meshScene" },
		{ 2, 0, _tr12, "dumpSceneVoxelPositions" },
		{ 2, 0, _tr13, "dumpSceneVoxelPositionsBlockwise" },
		{ 5, 0, _tr14, "dumpSceneOptimizationBlocks" },
		{ 4, 0, _tr15, "meshSceneWithShader" },
		{ 2, 0, _tr16, "initFineFromCoarse" },
		{ 6, 0, _tr17, "refineScene" },
		{ 6, 0, _tr18, "sceneEnergy" },
		{ 2, 0, _tr19, "computeArtificialLighting" },
		{ 1, 0, _tr20, "estimateLighting" },
		{ 2, 0, _tr21, "buildSphereScene" },
		{ 2, 2, _tr22, "renderScene" },
		{ 2, 2, _tr23, "processFrame" },
		{ 0, 0, _tr24, "assertFalse" },
		{ 0, 0, _tr25, "assertGPUFalse" }
		};
...
static const char* evalstrs[] = {
	...
	(const char*)0,
	"Begin@\"InfiniTAM`Private`\" (* create everythin in InfiniTAM`Priv",
	"ate`* *)",
	(const char*)0,
	"UnprotectClearAll@\"InfiniTAM`Private`*\" (* create everythin in I",
	"nfiniTAM`Private`* *)",
	(const char*)0,
	"Protect@\"InfiniTAM`Private`*\"",
	(const char*)0,
	"End[]",
	(const char*)0,
...
#endif
{
	...
	if (_res) _res = _definepattern(mlp, (char *)"createScene[voxelSize_Real]", (char *)"{ voxelSize }", 1);
	if (_res) _res = _definepattern(mlp, (char *)"getSceneVoxelSize[id_Integer?NonNegative]", (char *)"{ id }", 2);
	if (_res) _res = _definepattern(mlp, (char *)"sceneExistsQ[id_Integer?NonNegative]", (char *)"{ id }", 3);
	if (_res) _res = _definepattern(mlp, (char *)"initAD[id_Integer?NonNegative]", (char *)"{ id }", 4);
	if (_res) _res = _definepattern(mlp, (char *)"initFineADFromCoarseAD[fineid_Integer?NonNegative, coarseid_Integer?NonNegative] /; fineid != coarseid", (char *)"{ fineid, coarseid }", 5);
	if (_res) _res = _definepattern(mlp, (char *)"serializeScene[id_Integer?NonNegative, fn_String]", (char *)"{ id, fn }", 6);
	if (_res) _res = _definepattern(mlp, (char *)"deserializeScene[id_Integer?NonNegative, fn_String]", (char *)"{ id, fn }", 7);
	if (_res) _res = _definepattern(mlp, (char *)"countVoxelBlocks[id_Integer?NonNegative]", (char *)"{ id }", 8);
	if (_res) _res = _definepattern(mlp, (char *)"getVoxelBlock[id_Integer?NonNegative, i_Integer?Positive]", (char *)"{ id, i }", 9);
	if (_res) _res = _definepattern(mlp, (char *)"putVoxelBlock[id_Integer?NonNegative, voxelBlockData : { {_,_,_} (*pos*), {__List} (*8^3 voxels' data*) }]", (char *)"{ id, voxelBlockData }", 10);
	if (_res) _res = _definepattern(mlp, (char *)"meshScene[id_Integer?NonNegative, fn_String]", (char *)"{ id, fn }", 11);
	if (_res) _res = _definepattern(mlp, (char *)"dumpSceneVoxelPositions[id_Integer?NonNegative, fn_String]", (char *)"{ id, fn }", 12);
	if (_res) _res = _definepattern(mlp, (char *)"dumpSceneVoxelPositionsBlockwise[id_Integer?NonNegative, fn_String]", (char *)"{ id, fn }", 13);
	if (_res) _res = _definepattern(mlp, (char *)"dumpSceneOptimizationBlocks[id_Integer?NonNegative, fn_String, {offsetx_Integer,offsety_Integer,offsetz_Integer}]", (char *)"{ id, fn, offsetx, offsety, offsetz }", 14);
	...
	if (_res) _res = _definepattern(mlp, (char *)"initFineFromCoarse[idFine_Integer, idCoarse_Integer] /; (idFine != idCoarse)", (char *)"{ idFine, idCoarse }", 16);
	if (_res) _res = _definepattern(mlp, (char *)"refineScene[id_Integer, eg_Real, er_Real, es_Real, ea_Real, l : {__Real}]", (char *)"{ id, eg, er, es, ea, l }", 17);
	if (_res) _res = _definepattern(mlp, (char *)"sceneEnergy[id_Integer, eg_Real, er_Real, es_Real, ea_Real, l : {__Real}]", (char *)"{ id, eg, er, es, ea, l }", 18);
	if (_res) _res = _definepattern(mlp, (char *)"computeArtificialLighting[id_Integer?NonNegative, dir : {_,_,_}?numericVectorQ]", (char *)"{ id, dir}", 19);
	if (_res) _res = _definepattern(mlp, (char *)"estimateLighting[id_Integer?NonNegative]", (char *)"{ id }", 20);
	if (_res) _res = _definepattern(mlp, (char *)"buildSphereScene[id_Integer?NonNegative, rad_Real]", (char *)"{ id, rad }", 21);
	if (_res) _res = _definepattern(mlp, (char *)"renderScene[         id_Integer?NonNegative,          shader_String,         (* Manual *)         poseWorldToView_?PoseMatrixQ,         rgbIntrinsics : NamelessIntrinsicsPattern[]     ]", (char *)"{ id, shader, poseWorldToView, rgbIntrinsics }", 22);
	if (_res) _res = _definepattern(mlp, (char *)"processFrame[doTracking : 0|1     , id_Integer?NonNegative     (* Manual *)     , rgbaByteImage_ /;TensorQ[rgbaByteImage, IntegerQ] && Last@Dimensions@rgbaByteImage == 4     , depthData_?NumericMatrixQ     , poseWorldToView_?PoseMatrixQ     , intrinsicsRgb : NamelessIntrinsicsPattern[]     , intrinsicsD : NamelessIntrinsicsPattern[]     , rgbToDepth_?PoseMatrixQ     ]", (char *)"{ doTracking, id, rgbaByteImage, depthData, poseWorldToView, intrinsicsRgb, intrinsicsD, rgbToDepth }", 23);
	if (_res) _res = _definepattern(mlp, (char *)"assertFalse[]", (char *)"{ }", 24);
	if (_res) _res = _definepattern(mlp, (char *)"assertGPUFalse[]", (char *)"{ }", 25);
...
#endif
{
	return _WSDoCallPacket( mlp, _tramps, 26);
} /* WSDoCallPacket */
